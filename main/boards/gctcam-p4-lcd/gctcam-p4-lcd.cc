...
class GctcamP4Board : public WifiBoard {
private:
    Button boot_button_;
    ...
    static void touchpad_daemon(void *param)
    {
        ...
        auto &board = (GctcamP4Board &)Board::GetInstance();
    ...
    void InitializeSpi() {
        ...
        buscfg.mosi_io_num = GPIO_NUM_3;
        buscfg.miso_io_num = GPIO_NUM_NC;
        buscfg.sclk_io_num = GPIO_NUM_2;
    ...
    void InitializeSt7789Display() {
        ...
        ESP_LOGD(TAG, "Install LCD driver");
        esp_lcd_panel_dev_config_t panel_config = {};
        panel_config.reset_gpio_num = GPIO_NUM_0;
        panel_config.rgb_ele_order = LCD_RGB_ELEMENT_ORDER_RGB;
        panel_config.bits_per_pixel = 16;
        ...
        void init_sd()
    {
        ...
        // ESP_LOGI(TAG, "Initializing gctcam-p4-lcd Board");
...
public:
    GctcamP4Board() : boot_button_(BOOT_BUTTON_GPIO), clock_ui_(nullptr), clock_enabled_(false), clock_update_timer_(nullptr) {
        ESP_LOGI(TAG, "Initializing gctcam-p4-lcd Board");
        InitializeI2c();
        I2cDetect();
        init_sd();
    ...
    virtual void ShowClock() override {
        ...
        if (!alarm_manager.IsTimeValid()) {
            ...
            xTaskCreate([](void* param) {
                auto* board = static_cast<GctcamP4Board*>(param);
    ...
    static void ClockUpdateTimerCallback(void* arg) {
        auto* board = static_cast<GctcamP4Board*>(arg);
...
private:
    void ScheduleSmartTimeSync() {
        ...
        xTaskCreate([](void* param) {
            ...
            // 获取board指针
            auto* board_ptr = static_cast<GctcamP4Board*>(param);
        ...
        time_sync_manager.SetSyncCallback([board_ptr](bool success, const std::string& message) {
            ...
            esp_task_wdt_delete(NULL);
            vTaskDelete(nullptr);
        }, "smart_time_sync", 4096*2, this, 2, nullptr);  // 减少栈大小到4096，降低优先级
    }
...
// 静态变量定义
uint8_t* GctcamP4Board::network_music_buffer = nullptr;
size_t GctcamP4Board::network_music_size = 0;

DECLARE_BOARD(GctcamP4Board);
